...
public:
    ...
    void bulk_load(const V *vs, int num_keys)
    {
        ...
        destroy_tree(root);
        root = build_trie_bulk(keys, values, num_keys);
        delete[] keys;
        delete[] values;
    }
    ...
    void show() const
    {
        ...
        while (!s.empty())
        {
            ...
            printf("Node(%p, a = %g, b = %Lf, num_items = %d, layer = %d)", node,
                   node->model.a, node->model.b, node->num_items, node->layer);
            printf("[");
            int first = 1;
...
private:
    ...
    Node *build_tree_cp2(T key_cp1, T key_cp2, uint8_t layer, Node *subtree1,
                         Node *subtree2)
    {
        static_assert(BITMAP_WIDTH == 8);
        Node *node = NULL;
    ...
    Node *rebuild_tree_bulk(T *_keys, P *_values, int _size, bitmap_t *is_cp,
                            std::map<int, Node *> &nodes, uint8_t _layer,
                            int _treesize)
    {
        if (USE_FMCD)
        {
            return rebuild_tree_bulk_fmcd(_keys, _values, _size, is_cp, nodes, _layer,
                                          _treesize);
        }
        else
        {
            return rebuild_tree_bulk_fast(_keys, _values, _size, is_cp, nodes, _layer,
                                          _treesize);
        }
    }
    /// bulk build, _keys must be sorted in asc order.
    /// split keys into three parts at each node.
    Node *rebuild_tree_bulk_fast(T *_keys, P *_values, int _size, bitmap_t *is_cp,
                                 std::map<int, Node *> &nodes, uint8_t _layer,
                                 int _treesize)
    {
        RT_ASSERT(_size > 1);
        ...
        while (!s.empty())
        {
            ...
            if (end - begin == 2)
            {
                if (BITMAP_GET(is_cp, begin) == 0)
                {
                    ...
                    {
                        Node *_ =
                            build_tree_two(_keys[begin], _values[begin], _keys[begin + 1],
                                           _values[begin + 1], _layer);
                    ...
                    else
                    {
                        ...
                        Node *_ = build_tree_cp(_keys[begin], _values[begin],
                                                _keys[begin + 1], _layer, nodes[begin + 1]);
                        memcpy(node, _, sizeof(Node));
                        delete_nodes(_, 1);
                    }
                }
                else
                {
                    if (BITMAP_GET(is_cp, begin + 1) == 0)
                    {
                        ...
                        Node *_ = build_tree_cp(_keys[begin + 1], _values[begin + 1],
                                                _keys[begin], _layer, nodes[begin]);
                        memcpy(node, _, sizeof(Node));
                        delete_nodes(_, 1);
                    }
                    else
                    {
                        RT_ASSERT(nodes.find(begin) != nodes.end() &&
                                  nodes.find(begin + 1) != nodes.end());
                        Node *_ = build_tree_cp2(_keys[begin], _keys[begin + 1], _layer,
                                                 nodes[begin], nodes[begin + 1]);
                        memcpy(node, _, sizeof(Node));
                        delete_nodes(_, 1);
                    }
                }
            ...
            else
            {
                ...
                const long double mid1_key =
                    (static_cast<long double>(
                         keys[mid1_pos].to_model_key(node->layer)) +
                     static_cast<long double>(
                         keys[mid1_pos + 1].to_model_key(node->layer))) /
                    2;
                const long double mid2_key =
                    (static_cast<long double>(
                         keys[mid2_pos].to_model_key(node->layer)) +
                     static_cast<long double>(
                         keys[mid2_pos + 1].to_model_key(node->layer))) /
                    2;
                     ...
                     offset < size;)
                {
                    ...
                    else
                    {
                        ...
                        s.push((Segment){begin + offset, begin + next, level + 1,
                                         node->items[item_i].comp.child, ts});
                    }
                    if (next >= size)
                    {
                        break;
    ...
    /// bulk build, _keys must be sorted in asc order.
    /// FMCD method.
    Node *rebuild_tree_bulk_fmcd(T *_keys, P *_values, int _size, bitmap_t *is_cp,
                                 std::map<int, Node *> &nodes, uint8_t _layer,
                                 int _treesize)
    {
        RT_ASSERT(_size > 1);
        ...
        while (!s.empty())
        {
            ...
            else
            {
                ...
                // U_T. In fact, it has only a negligible impact of the performance.
                {
                    ...
                    while (i < size - 1 - D)
                    {
                        while (i + D < size && keys[i + D].to_model_key(node->layer) -
    ...
    Node *build_this_layer_tree_bulk_fast(T *_keys, P *_values, int _size,
                                          int *_kvs, bitmap_t *is_cp,
                                          uint8_t _layer, int treesize)
    {
        RT_ASSERT(_size > 1);
        ...
        while (!s.empty())
        {
            ...
            {
                int *kvs = _kvs;
                Node *_ = build_tree_two(_keys[kvs[begin]], _values[kvs[begin]],
                                         _keys[kvs[begin + 1]], _values[kvs[begin + 1]],
                                         _layer);
                memcpy(node, _, sizeof(Node));
                delete_nodes(_, 1);
                // check cp_NODE
                int _ts = 2;
                int _s = 0;
                if (BITMAP_GET(is_cp, begin) == 1)
                {
                    RT_ASSERT(begin < _size - 1)
                    int _pos = PREDICT_POS(node, _keys[kvs[begin]]);
                    BITMAP_SET(node->child_bitmap, _pos);
                    BITMAP_SET(node->cp_bitmap, _pos);
                    _s = kvs[begin + 1] - kvs[begin];
                    node->items[_pos].comp.child = build_trie_bulk_(
                        &_keys[kvs[begin]], &_values[kvs[begin]], _s, _layer + 1);
                    _ts = _ts - 1 + _s;
                }
                if (BITMAP_GET(is_cp, begin + 1) == 1)
                {
                    RT_ASSERT(begin + 1 < _size)
                    int _pos = PREDICT_POS(node, _keys[kvs[begin + 1]]);
                    BITMAP_SET(node->child_bitmap, _pos);
                    BITMAP_SET(node->cp_bitmap, _pos);
                    if (begin + 1 < _size - 1)
                    {
                        _s = kvs[begin + 2] - kvs[begin + 1];
                    }
                    else
                    {
                        _s = treesize - kvs[begin + 1];
                    }
                    node->items[_pos].comp.child = build_trie_bulk_(
                        &_keys[kvs[begin + 1]], &_values[kvs[begin + 1]], _s, _layer + 1);
                    _ts = _ts - 1 + _s;
                }
                node->size = _ts;
            }
            else
            {
                ...
                const long double mid1_key =
                    (static_cast<long double>(
                         keys[kvs[mid1_pos]].to_model_key(node->layer)) +
                     static_cast<long double>(
                         keys[kvs[mid1_pos + 1]].to_model_key(node->layer))) /
                    2;
                const long double mid2_key =
                    (static_cast<long double>(
                         keys[kvs[mid2_pos]].to_model_key(node->layer)) +
                     static_cast<long double>(
                         keys[kvs[mid2_pos + 1]].to_model_key(node->layer))) /
                    2;
                ...
                for (int item_i = PREDICT_POS(node, keys[kvs[0]]), offset = 0;
                     offset < size;)
                {
                    int next = offset + 1, next_i = -1;
                    while (next < size)
                    {
                        next_i = PREDICT_POS(node, keys[kvs[next]]);
                        if (next_i == item_i)
                        {
                            next++;
                        }
                    ...
                    {
                        BITMAP_CLEAR(node->none_bitmap, item_i);
                        node->items[item_i].comp.data.key = keys[kvs[offset]];
                        node->items[item_i].comp.data.value = values[kvs[offset]];
                        if (BITMAP_GET(is_cp, begin + offset) == 1) // is NODE_cp
                        {
                            RT_ASSERT(begin + offset < _size);
                            BITMAP_SET(node->child_bitmap, item_i);
                            BITMAP_SET(node->cp_bitmap, item_i);

                            int _s = 0;
                            if (begin + offset < _size - 1)
                            {
                                _s = kvs[offset + 1] - kvs[offset];
                            }
                            else
                            { // begin + offset = _size: the last key is cp_keys
                                _s = treesize - kvs[offset];
                            }

                            node->items[item_i].comp.child = build_trie_bulk_(
                                &keys[kvs[offset]], &values[kvs[offset]], _s, _layer + 1);
                        }
                    }
                    else
                    {
    ...
    Node *build_this_layer_tree_bulk_fmcd(T *_keys, P *_values, int _size,
                                          int *_kvs, bitmap_t *is_cp,
                                          uint8_t _layer, int treesize)
    {
        RT_ASSERT(_size > 1);
        ...
        while (!s.empty())
        {
            ...
            else
            {
...

                {
                    ...
                    int D = 1;
                    RT_ASSERT(D <= size - 1 - D);
                    double Ut = (static_cast<long double>(
                                     keys[kvs[size - 1 - D]].to_model_key(node->layer)) -
                                 static_cast<long double>(
                                     keys[kvs[D]].to_model_key(node->layer))) /
                                    (static_cast<double>(L - 2)) +
                                1e-6;
                    while (i < size - 1 - D)
                    {
                        while (i + D < size &&
                               keys[kvs[i + D]].to_model_key(node->layer) -
                                       keys[kvs[i]].to_model_key(node->layer) >=
                                   Ut)
                        {
                            i++;
                        }
    ...
    Node *build_trie_bulk(T *_keys, P *_values, int _size)
    {
        return build_trie_bulk_(_keys, _values, _size, 0);
    }
    Node *build_trie_bulk_(T *_keys, P *_values, int _size, uint8_t _layer)
    {
        Node *ret;
        const T *keys = _keys;
        const P *values = _values;
        const int size = _size;
        const uint8_t layer = _layer;
        int size_this_layer = 1;
        for (int i = 1; i < size; i++)
        {
            if (keys[i].to_model_key(layer) != keys[i - 1].to_model_key(layer))
                size_this_layer++;
        }
        if (size_this_layer == 1)
        {
            ret = build_trie_bulk_(_keys, _values, _size, _layer + 1);
        }
        else
        { // build the tree of this layer
            int *this_layer_kvs = new int[size_this_layer];
            bitmap_t *is_cp = new_bitmap(BITMAP_SIZE(size_this_layer));
            memset(is_cp, 0, sizeof(bitmap_t) * BITMAP_SIZE(size_this_layer));
            int j = 0;
            uint64_t temp = keys[0].to_model_key(layer);

            this_layer_kvs[0] = 0;
            for (int i = 1; i < size; i++)
            {
                if (keys[i].to_model_key(layer) != temp)
                {
                    this_layer_kvs[++j] = i;
                    temp = keys[i].to_model_key(layer);
                }
                else
                {
                    BITMAP_SET(is_cp, j);
            ...
            if (USE_FMCD)
                ret =
                    build_this_layer_tree_bulk_fmcd(_keys, _values, size_this_layer,
                                                    this_layer_kvs, is_cp, layer, size);
            else
                ret =
                    build_this_layer_tree_bulk_fast(_keys, _values, size_this_layer,
                                                    this_layer_kvs, is_cp, layer, size);
            delete[] this_layer_kvs;
            delete_bitmap(is_cp, BITMAP_SIZE(size_this_layer));
        }
    ...
    void scan_and_destory_tree(Node *_root, T *keys, P *values, bitmap_t *is_cp,
                               std::map<int, Node *> &nodes, bool destory = true)
    {
    ...
    Node *insert_tree(Node *_node, const T &key, const P &value)
    {
        ...
        for (Node *node = _node;;)
        {
            ...
            { // insert to DATA
                ...
                node->items[pos].comp.child =
                    build_tree_two(key, value, node->items[pos].comp.data.key,
                                   node->items[pos].comp.data.value, layer);
                insert_to_data = 1;
                if (layer != node->layer)
                {
            ...
            { // insert to NODE_cp
                ...
                else
                {
                    ...
                    {
                        node->items[pos].comp.child =
                            build_tree_cp(key, value, node->items[pos].comp.data.key, layer,
                                          node->items[pos].comp.child);
        ...
        {
            if (path[i]->layer == insert_layer)
            {
                path[i]->num_inserts++;
                path[i]->num_insert_to_data += insert_to_data;
                path[i]->size_this_layer++;
            }
        }
        ...
        for (int i = 0; i < path_size; i++)
        {
            ...
            const bool need_rebuild =
                node->fixed == 0 && node->layer == insert_layer &&
                node->size_this_layer >= node->build_size * 4 &&
                node->size_this_layer >= 64 && num_insert_to_data * 10 >= num_inserts;
...
#endif
                Node *new_node =
                    rebuild_tree_bulk(keys, values, ESIZE, is_cp, nodes, _layer, _size);
#if COLLECT_TIME
...
public:
    ...
    class SLIPPIterator
    {
        using slipp_type = SLIPP<T, P, USE_FMCD>;
    ...
    public:
        ...
        SLIPPIterator &operator++()
        {
            advance_stack();
            return *this;
        }

        SLIPPIterator operator++(int)
        {
            SLIPPIterator i(it);
            ++i;
            return i;
        ...
        bool operator==(const SLIPPIterator &rhs) const { return it == rhs.it; }
        bool operator!=(const SLIPPIterator &rhs) const { return it != rhs.it; }
    };
    using iterator = SLIPPIterator;
    ...
    iterator lower_bound(const T &key) const
    {
        ...
        while (true)
        {
            ...
            tuple.one_bitmap = ~(node->none_bitmap[tuple.index] |
                                 ((1 << ((pos % BITMAP_WIDTH) + 1)) - 1));
            it.stack.push(tuple);
            if (BITMAP_GET(node->child_bitmap, pos) == 1)
            {
                node = node->items[pos].comp.child;
